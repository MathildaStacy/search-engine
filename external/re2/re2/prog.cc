  ...
  switch (opcode()) {
    ...
    case kInstByteRange:
      return absl::StrFormat("byte%s [%02x-%02x] %d -> %d",
                             foldcase() ? "/i" : "", lo_, hi_, hint(), out());

    case kInstCapture:
      return absl::StrFormat("capture %d -> %d", cap_, out());

    case kInstEmptyWidth:
      return absl::StrFormat("emptywidth %#x -> %d", static_cast<int>(empty_),
                             out());
...
Prog::~Prog() {
  ...
  if (prefix_foldcase_) delete[] prefix_dfa_;
}
...
static uint64_t *BuildShiftDFA(std::string prefix) {
  ...
  for (size_t dcurr = 0; dcurr < size; ++dcurr) {
    for (uint8_t b : prefix) {
      ...
      uint16_t nnext = nfa[b] & ((ncurr << 1) | 1);
      size_t dnext = 0;
      while (states[dnext] != nnext) ++dnext;
...
const void *Prog::PrefixAccel_ShiftDFA(const void *data, size_t size) {
  ...
  while (p != endp) {
    ...
    curr = next >> (curr & 63);
    if ((curr & 63) == kShiftDFAFinal * 6) return p - prefix_size_;
  }
  return NULL;
}
