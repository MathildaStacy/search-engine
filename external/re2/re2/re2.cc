...
RE2::RE2(absl::string_view pattern) { Init(pattern, DefaultOptions); }
...
RE2::~RE2() {
  if (group_names_ != empty_group_names()) delete group_names_;
  if (named_groups_ != empty_named_groups()) delete named_groups_;
  delete rprog_;
  delete prog_;
  if (error_arg_ != empty_string()) delete error_arg_;
...
bool RE2::Replace(std::string *str, const RE2 &re, absl::string_view rewrite) {
  absl::string_view vec[kVecSize];
  int nvec = 1 + MaxSubmatch(rewrite);
  if (nvec > 1 + re.NumberOfCapturingGroups()) return false;
  if (nvec > static_cast<int>(ABSL_ARRAYSIZE(vec))) return false;
...
bool RE2::Extract(absl::string_view text, const RE2 &re,
...
bool RE2::Match(absl::string_view text, size_t startpos, size_t endpos,
  ...
  for (int i = ncap; i < nsubmatch; i++) submatch[i] = absl::string_view();
  return true;
}

// Internal matcher - like Match() but takes Args not string_views.
bool RE2::DoMatch(absl::string_view text, Anchor re_anchor, size_t *consumed,
...
// to string "out".
bool RE2::Rewrite(std::string *out, absl::string_view rewrite,
