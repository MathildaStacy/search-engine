...
class SubexpressionWalker : public re2::Regexp::Walker<int> {
 public:
  ...
  int PostVisit(re2::Regexp *re, int parent_arg, int pre_arg, int *child_args,
...
void TestOneInput(absl::string_view pattern, const RE2::Options &options,
  ...
  for (size_t i = 0; i < pattern.size(); i++) {
    if (pattern[i] == '.' || pattern[i] == 'k' || pattern[i] == 'K' ||
        pattern[i] == 's' || pattern[i] == 'S')
      char_class++;
    if (pattern[i] != '\\') continue;
    i++;
    if (i >= pattern.size()) break;
    if (pattern[i] == 'p' || pattern[i] == 'P' || pattern[i] == 'd' ||
        pattern[i] == 'D' || pattern[i] == 's' || pattern[i] == 'S' ||
        pattern[i] == 'w' || pattern[i] == 'W')
      char_class++;
    if (pattern[i] == 'p' || pattern[i] == 'P') backslash_p++;
  }
  if (char_class > 9) return;
  ...
  if (SubexpressionWalker().Walk(re.Regexp(), -1) > 9) return;
  ...
  if (SubstringWalker().Walk(re.Regexp(), -1) > 9) return;
