...
class Regexp::ParseState {
 public:
  ...
  bool ParseCCCharacter(absl::string_view *s, Rune *rp,
                        absl::string_view whole_class, RegexpStatus *status);
...
Regexp::ParseState::ParseState(ParseFlags flags, absl::string_view whole_regexp,
                               RegexpStatus *status)
    : flags_(flags),
...
bool Regexp::ParseState::PushLiteral(Rune r) {
  ...
  if (flags_ & FoldCase) {
    if (flags_ & Latin1 && (('A' <= r && r <= 'Z') || ('a' <= r && r <= 'z'))) {
...
void Regexp::RemoveLeadingString(Regexp *re, int n) {
  ...
  while (re->op() == kRegexpConcat) {
    if (d < ABSL_ARRAYSIZE(stk)) stk[d++] = re;
    re = re->sub()[0];
  }
...
void FactorAlternationImpl::Round3(Regexp **sub, int nsub,
  ...
  for (int i = 0; i <= nsub; i++) {
    ...
    if (i == start) {
      ...
      Regexp *re =
          Regexp::NewCharClass(ccb.GetCharClass(), flags & ~Regexp::FoldCase);
...
static bool ParseInteger(absl::string_view *s, int *np) {
  if (s->empty() || !absl::ascii_isdigit((*s)[0] & 0xFF)) return false;
...
static bool IsValidUTF8(absl::string_view s, RegexpStatus *status) {
  ...
  Rune r;
  while (!t.empty()) {
    if (StringViewToRune(&r, &t, status) < 0) return false;
  }
  return true;
}
...
static bool ParseEscape(absl::string_view *s, Rune *rp, RegexpStatus *status,
                        int rune_max) {
  ...
  if (StringViewToRune(&c, s, status) < 0) return false;
  int code;
  switch (c) {
    default:
    ...
    case 'x':
      ...
      if (s->empty()) goto BadEscape;
      if (StringViewToRune(&c1, s, status) < 0) return false;
...
static const UGroup *LookupGroup(absl::string_view name, const UGroup *groups,
                                 int ngroups) {
  // Simple name lookup.
  for (int i = 0; i < ngroups; i++)
    if (absl::string_view(groups[i].name) == name) return &groups[i];
  return NULL;
}
...
static const UGroup *LookupUnicodeGroup(absl::string_view name) {
  ...
  if (name == absl::string_view("Any")) return &anygroup;
...
ParseStatus ParseUnicodeGroup(absl::string_view *s,
  ...
  if (!StringViewToRune(&c, s, status)) return kParseError;
  if (c != '{') {
...
static bool IsValidCaptureName(absl::string_view name) {
  ...
  Rune r;
  while (!t.empty()) {
    if (StringViewToRune(&r, &t, NULL) < 0) return false;
    if (cc->Contains(r)) continue;
    return false;
  }
  return true;
}
...
bool Regexp::ParseState::ParsePerlFlags(absl::string_view *s) {
  ...
  if ((t.size() > 4 && t[2] == 'P' && t[3] == '<') ||
    ...
    if (end == absl::string_view::npos) {
      if (!IsValidUTF8(t, status_)) return false;
  ...
  for (bool done = false; !done;) {
    ...
    if (StringViewToRune(&c, &t, status_) < 0) return false;
    switch (c) {
      default:
        goto BadPerlOp;
...
Regexp *Regexp::Parse(absl::string_view s, ParseFlags global_flags,
  ...
  if (global_flags & Literal) {
    ...
    while (!t.empty()) {
      Rune r;
      if (StringViewToRune(&r, &t, status) < 0) return NULL;
    ...
    switch (t[0]) {
      ...
      case '(':
        ...
        if (ps.flags() & NeverCapture) {
          ...
          if (!ps.DoLeftParen(absl::string_view())) return NULL;
        }
        t.remove_prefix(1);  // '('
        break;
      ...
      case '*': {  // Zero or more.
        ...
        Rep:
          ...
          opstr = absl::string_view(
              opstr.data(), static_cast<size_t>(t.data() - opstr.data()));
