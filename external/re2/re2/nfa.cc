...
std::string NFA::FormatCapture(const char **capture) {
  ...
  for (int i = 0; i < ncapture_; i += 2) {
    if (capture[i] == NULL)
      ...
      s += absl::StrFormat("(%d,?)", capture[i] - btext_);
    else
      s += absl::StrFormat("(%d,%d)", capture[i] - btext_,
                           capture[i + 1] - btext_);
  }
  return s;
}
...
bool NFA::Search(absl::string_view text, absl::string_view context,
  ...
  if (prog_->anchor_start() && BeginPtr(context) != BeginPtr(text))
    return false;
  if (prog_->anchor_end() && EndPtr(context) != EndPtr(text)) return false;
  ...
  for (const char *p = text.data();; p++) {
    ...
    if (runq->size() == 0) {
      if (ExtraDebug) absl::FPrintF(stderr, "dead\n");
      break;
    }
