...
void PrefilterTree::AssignUniqueIds(NodeSet *nodes,
  ...
  for (int i = static_cast<int>(v.size()) - 1; i >= 0; i--) {
    ...
    if (prefilter == NULL || prefilter->op() != Prefilter::AND) continue;
    if (CanonicalNode(nodes, prefilter) != prefilter) continue;
    int id = prefilter->unique_id();
...
void PrefilterTree::PrintDebugInfo(NodeSet *nodes) {
  ...
  for (size_t i = 0; i < entries_.size(); i++) {
    ...
    LOG(ERROR) << "EntryId: " << i << " N: " << parents.size()
               << " R: " << regexps.size();
    for (int parent : parents) LOG(ERROR) << parent;
  }
  LOG(ERROR) << "Set:";
  for (NodeSet::const_iterator iter = nodes->begin(); iter != nodes->end();
       ++iter)
