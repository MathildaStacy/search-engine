...
DFA::State *DFA::WorkqToCachedState(Workq *q, Workq *mq, uint32_t flag) {
    ...
    switch (ip->opcode()) {
      case kInstAltMatch:
        ...
        if (kind_ != Prog::kManyMatch &&
          ...
          if (ExtraDebug) absl::FPrintF(stderr, " -> FullMatchState\n");
  ...
  if (n == 0 && flag == 0) {
    if (ExtraDebug) absl::FPrintF(stderr, " -> DeadState\n");
    return DeadState;
  }
...
DFA::State *DFA::CachedState(int *inst, int ninst, uint32_t flag) {
  ...
  if (it != state_cache_.end()) {
    if (ExtraDebug) absl::FPrintF(stderr, " -cached-> %s\n", DumpState(*it));
    return *it;
  }
  ...
  s->ninst_ = ninst;
  s->flag_ = flag;
  if (ExtraDebug) absl::FPrintF(stderr, " -> %s\n", DumpState(s));
...
inline bool DFA::InlinedSearchLoop(SearchParams *params) {
  ...
  State *s = start;
  if (ExtraDebug) absl::FPrintF(stderr, "@stx: %s\n", DumpState(s));

  if (s->IsMatch()) {
    matched = true;
    lastmatch = p;
    if (ExtraDebug) absl::FPrintF(stderr, "match @stx! [%s]\n", DumpState(s));
  ...
  while (p != ep) {
    if (ExtraDebug) absl::FPrintF(stderr, "@%d: %s\n", p - bp, DumpState(s));
  ...
  if (ExtraDebug) absl::FPrintF(stderr, "@etx: %s\n", DumpState(s));
  ...
  if (s->IsMatch()) {
    matched = true;
    lastmatch = p;
    if (ExtraDebug) absl::FPrintF(stderr, "match @etx! [%s]\n", DumpState(s));
    if (params->matches != NULL) {
      for (int i = s->ninst_ - 1; i >= 0; i--) {
        int id = s->inst_[i];
        if (id == MatchSep) break;
...
bool DFA::Search(absl::string_view text, absl::string_view context,
  ...
  *failed = false;
  ...
  if (ExtraDebug) absl::FPrintF(stderr, "start %s\n", DumpState(params.start));
...
bool Prog::SearchDFA(absl::string_view text, absl::string_view context,
  ...
  *failed = false;
  ...
  if (caret && BeginPtr(context) != BeginPtr(text)) return false;
  if (dollar && EndPtr(context) != EndPtr(text)) return false;
  ...
  if (match0) {
    if (reversed_)
      *match0 = absl::string_view(
          ep, static_cast<size_t>(text.data() + text.size() - ep));
    else
      *match0 =
