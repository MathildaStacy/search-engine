...
AtomTest atom_tests[] = {
    ...
    {// Test to make sure that any atoms that have another atom as a
     ...
     {"(abc123|abc|defxyz|ghi789|abc1234|xyz).*[x-z]+", "abcd..yyy..yyyzzz",
      "mnmnpp[a-z]+PPP"},
...
bool CheckExpectedAtoms(const char *atoms[], size_t n, const char *testname,
  ...
  for (size_t i = 0; i < n; i++) expected.push_back(atoms[i]);
  ...
  if (!pass) {
    ...
    for (size_t i = 0; i < expected.size(); i++) LOG(ERROR) << expected[i];
    LOG(ERROR) << "Found #atoms = " << v->atoms.size();
    for (size_t i = 0; i < v->atoms.size(); i++) LOG(ERROR) << v->atoms[i];
  }
...
TEST(FilteredRE2Test, AtomTests) {
  ...
  for (size_t i = 0; i < ABSL_ARRAYSIZE(atom_tests); i++) {
    ...
    for (nregexp = 0; nregexp < ABSL_ARRAYSIZE(t->regexps); nregexp++)
      if (t->regexps[nregexp] == NULL) break;
    for (natom = 0; natom < ABSL_ARRAYSIZE(t->atoms); natom++)
      if (t->atoms[natom] == NULL) break;
    AddRegexpsAndCompile(t->regexps, nregexp, &v);
    if (!CheckExpectedAtoms(t->atoms, natom, t->testname, &v)) nfail++;
  }
  EXPECT_EQ(0, nfail);
}
