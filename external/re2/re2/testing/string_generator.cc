...
bool StringGenerator::IncrementDigits() {
  ...
  for (int i = static_cast<int>(digits_.size()) - 1; i >= 0; i--) {
    if (++digits_[i] < static_cast<int>(alphabet_.size())) return true;
    digits_[i] = 0;
  }
...
bool StringGenerator::RandomDigits() {
  ...
  digits_.resize(len);
  for (int i = 0; i < len; i++) digits_[i] = random_alphabet_index(rng_);
  return true;
}
...
std::string DeBruijnString(int n) {
  ...
  for (size_t i = 0; i < static_cast<size_t>(n - 1); i++) s += '0';
  size_t bits = 0;
