...
bool Backtracker::Search(absl::string_view text, absl::string_view context,
  ...
  if (prog_->anchor_start() && BeginPtr(text) > BeginPtr(context_))
    return false;
  if (prog_->anchor_end() && EndPtr(text) < EndPtr(context_)) return false;
...
bool Backtracker::Visit(int id, const char *p) {
  ...
  if (visited_[n / 32] & (1 << (n & 31))) return false;
  visited_[n / 32] |= 1 << (n & 31);

  Prog::Inst *ip = prog_->inst(id);
  if (Try(id, p)) {
    if (longest_ && !ip->last()) Visit(id + 1, p);
    return true;
  }
  if (!ip->last()) return Visit(id + 1, p);
  return false;
}
...
bool Backtracker::Try(int id, const char *p) {
  ...
  int c = -1;
  if (p < text_.data() + text_.size()) c = *p & 0xFF;
  ...
  switch (ip->opcode()) {
    ...
    case kInstByteRange:
      if (ip->Matches(c)) return Visit(ip->out(), p + 1);
      return false;
    ...
    case kInstEmptyWidth:
      if (ip->empty() & ~Prog::EmptyFlags(context_, p)) return false;
      return Visit(ip->out(), p);
    ...
    case kInstMatch:
      ...
      if (endmatch_ && p != context_.data() + context_.size()) return false;
      cap_[1] = p;
...
bool Prog::UnsafeSearchBacktrack(absl::string_view text,
  ...
  if (kind == kFullMatch && EndPtr(match[0]) != EndPtr(text)) return false;
  return true;
}
