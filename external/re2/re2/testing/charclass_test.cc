...
static void Broke(const char *desc, const CCTest *t, CharClass *cc) {
  if (t == NULL) {
    ...
    absl::PrintF("\n");
    if (t->remove >= 0) absl::PrintF("Removed > %d\n", t->remove);
    absl::PrintF("\twant:");
...
bool ShouldContain(CCTest *t, int x) {
  for (int j = 0; t->final[j].lo >= 0; j++)
    if (t->final[j].lo <= x && x <= t->final[j].hi) return true;
  return false;
}
  ...
  for (int j = 0; j < 101; j++) {
    ...
    if (ShouldContain(t, j) == ncc->Contains(j)) {
      ...
      absl::PrintF("want ncc contains(%d)!=%d, got %d\n", j,
                   ShouldContain(t, j), ncc->Contains(j));
      Delete(ncc);
      return false;
    }
...
TEST(TestCharClassBuilder, Adds) {
  ...
  for (size_t i = 0; i < ABSL_ARRAYSIZE(tests); i++) {
    ...
    if (t->remove >= 0) ccb.RemoveAbove(t->remove);
    if (!CorrectCC(&ccb, t, "before copy (CharClassBuilder)")) nfail++;
    CharClass *cc = ccb.GetCharClass();
    if (!CorrectCC(cc, t, "before copy (CharClass)")) nfail++;
    cc->Delete();

    CharClassBuilder *ccb1 = ccb.Copy();
    if (!CorrectCC(ccb1, t, "after copy (CharClassBuilder)")) nfail++;
    cc = ccb.GetCharClass();
    if (!CorrectCC(cc, t, "after copy (CharClass)")) nfail++;
    cc->Delete();
    delete ccb1;
  }
  EXPECT_EQ(nfail, 0);
