...
BENCHMARK_RANGE(Search_Easy2_CachedNFA, 8, 256 << 10)
    ->ThreadRange(1, NumCPUs());
#ifdef USEPCRE
BENCHMARK_RANGE(Search_Easy2_CachedPCRE, 8, 16 << 20)
    ->ThreadRange(1, NumCPUs());
#endif
...
void BM_PCRE_Compile(benchmark::State &state) {
  RunBuild(state, absl::GetFlag(FLAGS_compile_regexp), CompilePCRE);
}
void BM_Regexp_Parse(benchmark::State &state) {
  RunBuild(state, absl::GetFlag(FLAGS_compile_regexp), ParseRegexp);
}
void BM_Regexp_Simplify(benchmark::State &state) {
  RunBuild(state, absl::GetFlag(FLAGS_compile_regexp), SimplifyRegexp);
}
void BM_CompileToProg(benchmark::State &state) {
  RunBuild(state, absl::GetFlag(FLAGS_compile_regexp), CompileToProg);
}
void BM_CompileByteMap(benchmark::State &state) {
  RunBuild(state, absl::GetFlag(FLAGS_compile_regexp), CompileByteMap);
}
void BM_Regexp_Compile(benchmark::State &state) {
  RunBuild(state, absl::GetFlag(FLAGS_compile_regexp), CompileRegexp);
}
void BM_Regexp_SimplifyCompile(benchmark::State &state) {
  RunBuild(state, absl::GetFlag(FLAGS_compile_regexp), SimplifyCompileRegexp);
}
void BM_Regexp_NullWalk(benchmark::State &state) {
  RunBuild(state, absl::GetFlag(FLAGS_compile_regexp), NullWalkRegexp);
}
void BM_RE2_Compile(benchmark::State &state) {
  RunBuild(state, absl::GetFlag(FLAGS_compile_regexp), CompileRE2);
}
...
void SearchDFA(benchmark::State &state, const char *regexp,
               absl::string_view text, Prog::Anchor anchor, bool expect_match) {
  for (auto _ : state) {
    Regexp *re = Regexp::Parse(regexp, Regexp::LikePerl, NULL);
    CHECK(re);
    Prog *prog = re->CompileToProg(0);
