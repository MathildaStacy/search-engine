...
static void PrintResult(const RE2 &re, absl::string_view input,
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (m[i].data() == NULL)
      ...
      absl::PrintF("%d-%d", BeginPtr(m[i]) - BeginPtr(input),
...
void ExhaustiveTester::HandleRegexp(const std::string &const_regexp) {
  ...
  if (LOGGING) {
    ...
    if (randomstrings_) LOG(ERROR) << "Cannot log with random strings.";
    if (regexps_ == 1) {  // first
      absl::PrintF("strings\n");
      strgen_.Reset();
      while (strgen_.HasNext()) absl::PrintF("%s\n", escape(strgen_.Next()));
      absl::PrintF("regexps\n");
    }
    absl::PrintF("%s\n", escape(regexp));
  ...
  if (tester.error()) return;

  strgen_.Reset();
  strgen_.GenerateNULL();
  if (randomstrings_) strgen_.Random(stringseed_, stringcount_);
  int bad_inputs = 0;
  while (strgen_.HasNext()) {
    tests_++;
    if (!tester.TestInput(strgen_.Next())) {
      failures_++;
      if (++bad_inputs >= absl::GetFlag(FLAGS_max_bad_regexp_inputs)) break;
    }
  }
}
...
void ExhaustiveTest(int maxatoms, int maxops,
                    ...
                    const std::string &wrapper, const std::string &topwrapper) {
  if (RE2_DEBUG_MODE) {
    if (maxatoms > 1) maxatoms--;
    if (maxops > 1) maxops--;
    if (maxstrlen > 1) maxstrlen--;
  }
  ...
  if (!LOGGING) {
    absl::PrintF("%d regexps, %d tests, %d failures [%d/%d str]\n", t.regexps(),
                 t.tests(), t.failures(), maxstrlen, stralphabet.size());
  }
