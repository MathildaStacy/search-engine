...
static uint32_t Engines() {
  ...
  if (did_parse) return cached_engines;

  if (absl::GetFlag(FLAGS_regexp_engines).empty()) {
    cached_engines = ~0;
  } else {
    for (Engine i = static_cast<Engine>(0); i < kEngineMax; i++)
      if (absl::GetFlag(FLAGS_regexp_engines).find(EngineName(i)) !=
          std::string::npos)
        cached_engines |= 1 << i;
  }

  if (cached_engines == 0) LOG(INFO) << "Warning: no engines enabled.";
  if (!UsingPCRE) cached_engines &= ~(1 << kEnginePCRE);
  for (Engine i = static_cast<Engine>(0); i < kEngineMax; i++) {
    if (cached_engines & (1 << i)) LOG(INFO) << EngineName(i) << " enabled";
  }
...
struct TestInstance::Result {
  ...
  void ClearSubmatch() {
    for (int i = 0; i < kMaxSubmatch; i++) submatch[i] = absl::string_view();
  }
...
static std::string FormatCapture(absl::string_view text, absl::string_view s) {
  if (s.data() == NULL) return "(?,?)";
  return absl::StrFormat("(%d,%d)", BeginPtr(s) - BeginPtr(text),
...
static bool NonASCII(absl::string_view text) {
  for (size_t i = 0; i < text.size(); i++)
    if ((uint8_t)text[i] >= 0x80) return true;
  return false;
}
...
static std::string FormatMode(Regexp::ParseFlags flags) {
  for (size_t i = 0; i < ABSL_ARRAYSIZE(parse_modes); i++)
    if (parse_modes[i].parse_flags == flags) return parse_modes[i].desc;
...
TestInstance::TestInstance(absl::string_view regexp_str, Prog::MatchKind kind,
  ...
  if (absl::GetFlag(FLAGS_dump_prog)) {
    LOG(INFO) << "Prog for "
              << " regexp " << absl::CEscape(regexp_str_) << " ("
              << FormatKind(kind_) << ", " << FormatMode(flags_) << ")\n"
              << prog_->Dump();
  }
  ...
  if (Engines() & ((1 << kEngineDFA) | (1 << kEngineDFA1))) {
    ...
    if (absl::GetFlag(FLAGS_dump_rprog)) LOG(INFO) << rprog_->Dump();
  }
  ...
  if (!(flags & Regexp::OneLine)) re = "(?m)" + re;
  if (flags & Regexp::NonGreedy) re = "(?U)" + re;
  if (flags & Regexp::DotNL) re = "(?s)" + re;
  ...
  if (Engines() &
    ...
    if (kind_ == Prog::kLongestMatch) options.set_longest_match(true);
  ...
  if ((Engines() & (1 << kEnginePCRE)) && regexp_->MimicsPCRE() &&
    ...
    if (flags & Regexp::Latin1) o.set_option(PCRE::None);
...
void TestInstance::RunSearch(Engine type, absl::string_view orig_text,
  ...
  if (nsubmatch > kMaxSubmatch) nsubmatch = kMaxSubmatch;
  ...
  switch (type) {
    ...
    case kEngineRE2b: {
      ...
      if (kind_ == Prog::kFullMatch) re_anchor = RE2::ANCHOR_BOTH;

      result->matched = re2_->Match(
          context, static_cast<size_t>(BeginPtr(text) - BeginPtr(context)),
          static_cast<size_t>(EndPtr(text) - BeginPtr(context)), re_anchor,
          result->submatch, nsubmatch);
    ...
    case kEnginePCRE: {
      ...
      if (kind_ == Prog::kFullMatch) pcre_anchor = PCRE::ANCHOR_BOTH;
      re_->ClearHitLimit();
      result->matched =
  ...
  if (!result->matched) result->ClearSubmatch();
}
...
static bool ResultOkay(const Result &r, const Result &correct) {
  ...
  if (r.matched != correct.matched) return false;
...
bool TestInstance::RunCase(absl::string_view text, absl::string_view context,
  ...
  for (Engine i = kEngineBacktrack + 1; i < kEngineMax; i++) {
    ...
    if (i == kEnginePCRE && NonASCII(text)) continue;

    if (!r.untrusted) all_okay = false;
    ...
    for (int i = 0; i < 1 + num_captures_; i++) {
      if (r.submatch[i].data() != correct.submatch[i].data() ||
        ...
        LOG(INFO) << absl::StrFormat("   $%d: should be %s is %s", i,
                                     FormatCapture(text, correct.submatch[i]),
                                     FormatCapture(text, r.submatch[i]));
      } else {
        LOG(INFO) << absl::StrFormat("   $%d: %s ok", i,
                                     FormatCapture(text, r.submatch[i]));
      }
    }
  }
...
void TestInstance::LogMatch(const char *prefix, Engine e,
                            ...
                            Prog::Anchor anchor) {
  LOG(INFO) << prefix << EngineName(e) << " regexp "
            << absl::CEscape(regexp_str_) << " "
            << absl::CEscape(regexp_->ToString()) << " text "
            << absl::CEscape(text) << " (" << BeginPtr(text) - BeginPtr(context)
            << "," << EndPtr(text) - BeginPtr(context) << ") of context "
            << absl::CEscape(context) << " (" << FormatKind(kind_) << ", "
            << FormatAnchor(anchor) << ", " << FormatMode(flags_) << ")";
}
