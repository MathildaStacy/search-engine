...
struct SetHooks {
  SetHooks() {
    hooks::SetDFAStateCacheResetHook(
        [](const hooks::DFAStateCacheReset &) { ++state_cache_resets; });
    hooks::SetDFASearchFailureHook(
        [](const hooks::DFASearchFailure &) { ++search_failures; });
  }
} set_hooks;
...
TEST(Multithreaded, BuildEntireDFA) {
  ...
  std::string s = "a";
  for (int i = 0; i < absl::GetFlag(FLAGS_size); i++) s += "[ab]";
  s += "b";
  ...
  for (int i = 0; i < absl::GetFlag(FLAGS_repeat); i++) {
    ...
    for (int j = 0; j < absl::GetFlag(FLAGS_threads); j++) threads[j].join();
...
TEST(SingleThreaded, SearchDFA) {
  ...
  Regexp *re =
      Regexp::Parse(absl::StrFormat("0[01]{%d}$", n), Regexp::LikePerl, NULL);
  ASSERT_TRUE(re != NULL);
