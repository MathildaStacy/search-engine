...
KeywordLRUCache::KeywordLRUCache(int capacity) : lru_size(capacity) {}


KeywordLRUCache::KeywordLRUCache(const KeywordLRUCache& other) {
...
// 拷贝赋值运算符
KeywordLRUCache& KeywordLRUCache::operator=(const KeywordLRUCache& other) {
...
std::vector<std::string> KeywordLRUCache::get(const std::string& key) {
    std::lock_guard<std::mutex> lock(mtx); // 上锁
    if (lru.find(key) != lru.end()) {
        auto it = lru[key];

        auto temp = *it;
        std::vector<std::string> value = std::get<1>(*it);
...
void KeywordLRUCache::put(const std::string& key, const std::vector<std::string>& value) {
    std::lock_guard<std::mutex> lock(mtx); // 上锁
    if (lru.find(key) != lru.end()) {
        auto it = lru[key];
...
std::vector<std::tuple<std::string, std::vector<std::string>>> KeywordLRUCache::getAllPairs() {
...
size_t KeywordLRUCache::get_size() {
    return lru_size;
}